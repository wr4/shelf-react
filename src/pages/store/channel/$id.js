...
const RelateChannel = (props) => {
    ...
    useEffect(() => {
        fetch(`http://ethles.com/api/goods/${id}`)
            ...
            .then(() => {
                fetch(`http://ethles.com/api/goods/channel/count?related_id=${related_id}`)
                    .then(res => res.json())
                    ...
                    .then(() => {
                        fetch(`http://ethles.com/api/goods/channel?limit=${pagination.pageSize}&offset=${0}&related_id=${related_id}`)
                            .then(res => res.json())
                            .then(arr => {
                                return arr.map(data => {

                                    if (!data.sigma_account_us_user) {
                                        return {
                                            channel_name: '',
                                        ...
                                        return {
                                            channel_name: data.sigma_account_us_user.name,
                                            id: data.id,
                                            available: data.available,
                                            subject: data.subject,
                                            body: data.body,
    ...
    const handleTableChange = (newPagination, filters, sorter) => {
        ...
        fetch(`http://ethles.com/api/goods/channel?limit=${pagination.pageSize}&offset=${pagination.current >= 1 ? (pagination.current - 1) * pagination.pageSize : 0}&related_id=${related_id}`)
            .then(res => res.json())
            .then(arr => {
                return arr.map(data => {
